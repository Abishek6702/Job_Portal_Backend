...
exports.login = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user._id, role: user.role, onboardingstatus: user.onboardingCompleted}, process.env.JWT_SECRET, { expiresIn: '1h' });
...
exports.registerInstructor = async (req, res) => {
  ...
  try {
    ...
    const newUser = new User({ name, email, phone, password, role: 'instructor' });
    await newUser.save();
    ...
    await sendEmail(email, 'Verify your Email', `Your OTP is: ${otp}`);
    res.status(201).json({ message: 'Instructor account created. OTP sent for verification.' });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
exports.loginInstructor = async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.isVerified) {
      return res.status(400).json({ message: 'Instructor not verified or does not exist' });
    }
